...
1. **Ephemeral data:** Once data is read from a FIFO, it's gone (unlike files where data persists)
...
1. **Complex data flows:** FIFOs are useful for facilitating complex data flows, merging and splitting data in flexible ways, and so on. For example, FIFOs support multiple readers and writers.
